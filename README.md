...
### DEFAULT SCENARIOS
**LIRI** features default programming to return results in the event a user enters a _COMMAND_ but fails to enter a subsequent _SEARCH_ term, or if the user fails to enter a _COMMAND_ at all.  These defaults results are demonstrated for each scenario below:
